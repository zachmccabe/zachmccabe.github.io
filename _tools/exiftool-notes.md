...
Copies file name and XMP description tag from `foo.mp4` to a TXT file with the same name. (If foo.txt already exists, the `!` operator overwrites the file):
...
If you are using ExifTool in the current directory, include a `.` period at the end of the line. Otherwise, you need to include your `path/to/dir`.
...
This will display both the file name and the color of the label tag. For MacOS, remember to add the single quotes because `$` sigil is used.
...
Note: The `i` in `/Red/i` makes if statement case insensitive. However, Photo Mechanic is picky and only recognizes XMP label if it is capitalized. Use Red, Yellow, Green, Blue, Purple, Cyan, Brown, Trash, None if you're writing to file. I'm not sure about other apps like Bridge, C1.
...
For MacOS, remember to add the single quotes because `$` sigil is used.
...
For coordinates in the southern or western hemisphere, you can either use `-` negative symbol before your coordinate, or use `S` or `W` after your coordinate. E.g.:
...
For example, this beastly snippet does all of the above, and also copies `UserData:description` and appends it to `xmp-dc:description` tag. (I.e. "XMP tag content...UserData tag content". I'm using an ellipses here but format it however you like.):
